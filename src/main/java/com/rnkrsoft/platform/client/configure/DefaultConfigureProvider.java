...
public class DefaultConfigureProvider implements ConfigureProvider {
    ...
    public Configure load(String schema, String host, int port, String contextPath, List<String> channels, String uic, String deviceType, String appVersion, double lat, double lng) {
        ...
        //通过线程池提交任务
        Future<FetchConfigureResponse> future = threadPoolExecutor.submit(new Callable<FetchConfigureResponse>() {
            @Override
            public FetchConfigureResponse call() throws Exception {
                return configureService.fetchConfigure(request);
            }
        });

        try {
            response = future.get(30, TimeUnit.SECONDS); //取得结果，同时设置超时执行时间为5秒。同样可以用future.get()，不设置执行超时时间取得结果
