    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        if (args.length == 1) {
            ...
            if (serviceConfigure.isDebug()) {
                serviceConfigure.log("sync call {}.{} ", serviceClass.getName(), method.getName());
            }
            ...
            if (method.getParameterTypes()[1].isAssignableFrom(AsyncHandler.class)) {
                ...
                if (serviceConfigure.isDebug()) {
                    serviceConfigure.log("async call {}.{} ", serviceClass.getName(), method.getName());
                    serviceConfigure.log("hread pool corePoolSize:'{}' activeCount:'{}' taskCount:'{}'", THREAD_POOL_EXECUTOR.getCorePoolSize(), THREAD_POOL_EXECUTOR.getActiveCount(), THREAD_POOL_EXECUTOR.getTaskCount());
                }
                Future future = THREAD_POOL_EXECUTOR.submit(new AsyncInvoker(serviceConfigure.getSessionId(), serviceConfigure, serviceClass, method.getName(), requestClass, responseClass, request, asyncHandler));
                if (serviceConfigure.isDebug()) {
                    serviceConfigure.log("async submit success ");
                }
